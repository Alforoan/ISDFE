...
const AccountSetUpUsers = () => {
    ...
    const [invitations, setInvitations] = useState([
            <Flex gap={"middle"}>
                ...
                <Form.Item label="Role" name="2" >
                    <Select options={options} style={{height:48,width:270}} placeholder={"Select Role"}/>
                </Form.Item>
            </Flex>
    ]);
    const addTeammates=()=>{
            let newInvitation=<Flex gap={"middle"}>
    ...
    return (
        <div className="form-container account-setup">
            <Form className={"form invitation-input"}  layout="vertical" onFinish={sendInvitations} size={"large"} name={"invitationsList"}>
                <h3 className="form-title">Instructional Design Workspace</h3>
                <h4 className="form-subtitle">Let’s invite the users and assign roles</h4>
                {
                ...
                <label onClick={addTeammates} style={{cursor:"pointer",color:"#0774c3",marginRight:"auto"}}>+ Add teammates</label>
